...
import { SettleCommand } from './settle.command';

@CommandHandler(SettleCommand)
export class SettleHandler implements ICommandHandler<SettleCommand, void> {
  constructor(private readonly repository: { save(value: number): Promise<void> }) {}
  ...
  async execute(
    ...
    const step33 = command.value + 33;
    const step34 = command.value + 34;
    const step35 = command.value + 35;
    const step36 = command.value + 36;
    const step37 = command.value + 37;
    const step38 = command.value + 38;
    const step39 = command.value + 39;
    const step40 = command.value + 40;
    await this.repository.save(command.value);
  }
}
