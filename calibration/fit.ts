...
export function renderReport(pin: string, rulebookVersion: string, generatedAt: string, fits: FitResult[]): string {
  ...
  for (const fit of fits) {
    ...
    lines.push(`## ${m.ruleId}`);
    lines.push('');
    lines.push(`Primitive ${m.primitive}, ${m.nGood} good and ${m.nBad} bad cases, ${m.errors} error(s), ${m.inputTokens} input tokens, uncertain rate ${pct(m.uncertainRate)}. Cuts restricted to >= hi=${fit.floor}; advise at the lowest cut of the best-F1 plateau, ask and deny at the highest cut of their plateau.`);
